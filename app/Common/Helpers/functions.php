 ...
 * @version   $Id$
 */
if (! function_exists('generateRndNames')) {
    /**
     * 生成随机名字
     *
     * @param int $num
     * @return array
     */
    function generateRndNames($num = 100) {
        $names = [];
        ...
        while (count($names) < $num) {
            $surname = $arrSurnames[mt_rand(0, count($arrSurnames) - 1)];
            $nameSize = rand(1, 2);
            $name = $arrNames[mt_rand(0, count($arrNames) - 1)];
            if ($nameSize == 2) {
                $name .= $arrNames[mt_rand(0, count($arrNames) - 1)];
            }
